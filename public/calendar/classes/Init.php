...
include_once ($_SERVER['DOCUMENT_ROOT'].'../../classes/dbConnPDOtests.php');
...
class Init extends dbConnPDO
{
	...
	// $path on the local environment equates to: C:\laragon\www\[domain root]
	...
	function __construct()
	{
		...
		$this->pdoDB = parent::dbOpen('custo299_calendar_tests');		
		...
		$this->cMo = date('m');
		$this->cYr = date('Y'); //4 digit representation of current yr as str.
		$this->activeOrdinalYear = (int)$this->cYr;
		...
		$this->company = isset($_POST['company']) ? trim($_POST['company']) : 'Custom Sign Center';
		...
		//define the xml path:
		//$this->path = absolute server path to the [app root]/classes directory:
		if( preg_match('~laragon~', $this->path) ) {
			//  page loading from local development server = true.
			//	when ran on the x drive IIS server, it equates to:			
			//	C:\laragon\www\....models
