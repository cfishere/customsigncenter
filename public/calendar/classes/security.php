...
class userAuthenticate extends Session {	
	...
	public function __construct(){
		...
		if(	$this->pdo = parent::dbOpen('custo299_calendar_tests')){
			$this->connected = TRUE;
			//is app in maintenance mode?
			$_SESSION['app']['maintenance'] = $this->maintenanceMode();
			if( $_SESSION['app']['maintenance'] )
			{
				header('Location: https://'.$_SERVER['SERVER_NAME'].'/maintenance.php');
				exit;
			}	
	...
	{
		$statement = $this->pdo->prepare('SELECT `maintenance` FROM app' );		
		$statement->execute();
		$statement->bindColumn('maintenance', $m); 
	...
	public function unleaseToken( $userId ) {		
		if( !is_object( $this->pdo ) ) {			
			...
			//connect to db			
			if( $this->pdo = parent::dbOpen('custo299_calendar_tests') ){//open connection to wipcalendar db.				
				$this->connected = TRUE;				
			} else {
