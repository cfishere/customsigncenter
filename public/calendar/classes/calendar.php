...
class calendar {
	...
	public $save;
	 ...
	 {
	 	/*exit('Calendar.php __construct called.');*/
	 	$this->init = new Init;
		/*exit('Calendar.php __construct called.');*/

		if( $this->init->method==NULL || empty( $this->init->method ) )
		{
			exit('Calendar.php __construct reports Init class\'s $method property is null or empty.');
			$this->display;
		} 
		else 
		{				
			...
			if( $action === 'saveMonth' )
			{
				exit('Calendar.php __construct, $action = saveMonth.');
				$this->save = new Save;
				$this->save->month();
			}
			else
			{
				exit('Calendar.php __construct reports the user request\'s method is not being processed correctly by Init.');
...
// display the company calendar
 ...
 { 	
	 ...
	 {
		 //Must know the active year in order to set the current data properly for multi-yr xmls.
		    ...
		    foreach($searchMonthNode as $searchMonth) //foreach1
		    {
			    ...
			    if((int)$this->init->activeMonthOrdinal === (int)$searchMonth->getAttribute('ordinal')
				   && (int)$yrNODE->getAttribute('ordinal') === (int)$this->init->activeOrdinalYear)
			    { 
		    ...
		    {				    
			    ...
			    for( $t = 0; $dayNode = $searchNode->getElementsByTagName('day')->item($t); $t++ )
			    {
			    	...
			    	if( $t===0 )
			    	{
			    		$viewWeekInModal = '<img onclick="modalOpen(this,\'week\')" src="assets/view-week-btn.png" title="view and print this week" class="modalImg" id="Week-' . (int)($wkKey+1) . '" >';
			    	}
				    ...
				    else  // else1
				    {
					    ...
					    if((int)$yrNODE->getAttribute('ordinal') === (int)$this->init->activeOrdinalYear &&
						   (int)$this->init->activeOrdinalCell === (int)$dayNode->getAttribute("ordinal") && 
						   (int)$this->init->activeMonthOrdinal === (int)$searchMonth->getAttribute('ordinal')) //if2
					    {
