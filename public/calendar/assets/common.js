...
iconAssignments={"Custom Sign Center":{1:"Will",/*2:"Ali",*/3:"Mary",/*4:"Marshall",*/5:"Jack",6:"SubInstall",7:"CSC Transp",8:"Shipping",9:"Cust PU",10:"UPS/ic-ups",11:"UnAssigned/ic-flag",12:"Warranty/ic-shield"},
"Outdoor Images":{1:"Chad",2:"Marvin",3:"Ash",4:"Tom",5:"Pat",6:null,7:"Manny",/*7:"Rec CSC Transp",8:"Rec Shipping"*/9:"Cust PU",/*10:"UPS/ic-ups",*/11:"UnAssigned/ic-flag",12:"Warranty/ic-shield"},
"Marion Signs":{},"Boyer Signs":{},"JG Signs":{}};
	...
	var overdueJobs = [];
	//add/remove jobs that are marked 'Ready-Ship' so they can be output to the view in designated areas.
	var shipreadyJobs = [];	
	//2 idle user globals
...
function recreateCheckboxListeners() {
	...
	var checkboxes = $("#second-chkbx-wrapper,#checkboxWrapper,#third-chkbx-wrapper").find("input");	
...
function loadCalendar(co, m, y){	
	...
	//reset the readyship jobs obj to empty.
	shipreadyJobs = [];
...
function teamsShowAll(){
	...
	if( $('#modal').is(":visible") === false ){			
		...
		var JobStatusInputsWithCheckboxes = $(".j, #second-chkbx-wrapper, #third-chkbx-wrapper").find("input");
...
function teamsHideAll(){
	...
	//hide every li item that contains an span->i-> with class beginning with "ico-"	
	//and any checkbox controllers in the secondary & tertiary column checkboxes #second-chkbx-wrapper, etc.
	var JobStatusInputsWithCheckboxes = $(".j, #second-chkbx-wrapper, #third-chkbx-wrapper").find("input");
	$(JobStatusInputsWithCheckboxes).each(function(){
		//uncheck ea box
...
function printAnyElement(elemId) {
	...
	//$( "#print .smBtn" ).addClass( "hide" );	
	 ...
	 printArea.document.write('<html><head><title>Print Request</title><link rel="stylesheet"/styles/bootstrap.min.css" media="all"><link href="/styles/print_1.css" media="print" rel="stylesheet" /> <link rel="stylesheet" href="/assets/icomoon/style.css">');
...
function onloadSetOverdueDisplay(){	
	...
	if( overdueJobs.length > 0 ){		
		...
		//by iterating through the overdue jobs properties:	
...
//tests whether obj is empty returns true if empty. 
//TODO: Combine this func with onloadSetOverdueDisplay
function onloadSetShipreadyDisplay(){	
	/*console.log('onloadSetShipreadyDisplay called in common.js');*/
	shipreadyJobs=[];
	const readyshipWrapper = document.getElementById('shipreadyJobsList');
	readyshipWrapper.innerHTML ='';
	
	wait('start'); //spinner gif indicating busy
	var $liReady = $("#weeks").find("li.ready");	
	wait('end'); 
	let checkDuplicateJobNumbers = [];
	$.each($liReady, function(i, d){
		
		//1. list this in the view of ship ready jobs.
		// the job number parent span is always the first one in the LI.		
			var jobNmbr = $(this).children('span').first().text();
			jobNmbr = jobNmbr.split(' ')[0]; //sometimes a trailing ' [CONT.]' on job numbers throws an error in js. Remove it. 
		...
		//2. get the date of the edited job's UL wrapper's modalid.
			...
			span.setAttribute('style','color: red');
			span.innerHTML = mo + '/' + dayOfMo + '/' + yr;*/
		//3. get the job desc text and truncate it to the first 30 chars.			v
			var div = document.createElement('div');			
			div.innerHTML += $(this).text().slice(0,30);
			div.id = jobNmbr;
			div.class = 'shpReady';
			/*shipreadyJobs.push({markup:div, dueDate: null, jobNmbr: jobNmbr});*/
			if( shipreadyJobs.hasOwnProperty(jobNmbr) === false )
			{
				shipreadyJobs[jobNmbr] = div;
			}
	});	
 ...
 */
function renewTeamIconCheckboxDisplay()
{
