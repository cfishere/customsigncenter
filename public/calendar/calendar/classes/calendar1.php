...
/*header('Access-Control-Allow-Origin: http://outdoorimagesinc.net', false);
header('Access-Control-Allow-Origin: http://customsigncenter.com', false);*/
...
class calendar {
	...
	/*	if( preg_match('~wwwroot~', $this->path) ) {
		...
		if(!is_writable ( $this->xmlPath.$this->xmlfile )){
			$m = "Error: File Not Writable: ".$this->xmlPath.$this->xmlfile;
			$msg = array('msg'=>$m);
			...
			{	
				...
				for( $i = 0; $yrNode = $this->companyCalendar->getElementsByTagName("year")->item($i); $i++ ) {		    		
					...
					if( (integer)$yrNode->getAttribute("ordinal") === (integer)$this->year ){
						...
						/*
					Found a year node and it is 2018Found a year node and it is 2019
Recoverable fatal error: Object of class DOMElement could not be converted to string in /home/custo299/public_html/calendar/classes/calendar1.php on line 163 when echoing the below
*/
						//echo '$this->activeYearNode equates to'. $this->activeYearNode->getAttribute("ordinal");
					}
			...
			$m = 'The xml document does not exist.';
			$msg = array('msg'=>$m);
			echo json_encode($msg);	
			exit;  
		}
										...
										if( $childUL->getElementsByTagName( "li" )->length > 0 ) {
											...
											// DEFINE THE jobNmbr VARIABLE AS THE 1st SPAN CHILD
												...
												if( $span->length ) {
													...
													}// if get 1st span's "id" attrib failed.
		   ...
		   for( $xyr = 0; $XMLyr = $this->companyCalendar->getElementsByTagName( 'year' )->item( $xyr ); $xyr++ ) {
			    ...
			    for( $i = 0; $m = $XMLyr->getElementsByTagName( 'month' )->item( $i ); $i++ ) {
				    ...
				    for( $s = 0; $d = $m->getElementsByTagName( "day" )->item( $s ); $s++ ) {
						...
						echo 'currentXMLyr is '. $currentXMLyr. ' and currentXMLmo is '. $currentXMLmo .' and currentXMLday is '.$currentXMLday;
